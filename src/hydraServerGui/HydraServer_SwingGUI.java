...
public class HydraServer_SwingGUI extends HydraServerGui {
	...
	public HydraServer_SwingGUI(HydraServer hydraServer) {
		this.hydraServer = hydraServer;
		JFrame window = new JFrame(hydraServer.serverName);
		try {
...
//		testPanel.add(addBtn);
	...
	public void refreshClientPanel() {
		ConcurrentHashMap<String, HydraConnectionClient> clients = this.hydraServer.getRepository().getClients();
		...
		for (String clientId : clients.keySet()) {
			...
			model.addRow(new Object[] { this.clientListTable.getRowCount() + 1, clientId,
